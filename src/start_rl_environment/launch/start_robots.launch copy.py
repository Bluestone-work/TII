...
def launch_setup(context, *args, **kwargs):
    ...
    # 映射 map_number 到文件名
    # 根据你提供的文件列表，这里的映射需要对应 maps 文件夹下的文件名
    map_mapping = {
        '1': 'map1', 
        '2': 'map2', 
        '3': 'corridor_swap', 
    ...
    print(f"[INFO] Spawning {robot_number} robots on {map_name}...")
    ...
    presets_path = os.path.join(pkg_share, 'config', 'spawn_presets.yaml')
    spawn_boxes = [[-2.0, -2.0, 2.0, 2.0]] # 默认的一个大框
    
    if os.path.exists(presets_path):
        with open(presets_path, 'r') as f:
            presets = yaml.safe_load(f)
            # 注意：spawn_presets.yaml 的 key 可能和 map_name 不完全一致，需要检查文件内容
            # 这里假设 yaml 结构是 {map_name: {start_regions: [[x1,y1,x2,y2], ...]}}
            if map_name in presets and 'start_regions' in presets[map_name]:
    ...
    for i in range(robot_number):
        ...
        # 显式构建参数字典
        args_dict = {
            'robot_name': f'my_bot{i}',
            'robot_namespace': f'my_bot{i}',
            'robot_name_prefix': f'my_bot{i}/',
            'x': str(x),
            'y': str(y),
            'z': '0.1',
            'rotation': str(yaw),
        ...
        tf_node = Node(
            ...
            name=f'static_tf_my_bot{i}',
            arguments=['0', '0', '0', '0', '0', '0', 'map', f'my_bot{i}/odom'],
            output='screen'
        )
