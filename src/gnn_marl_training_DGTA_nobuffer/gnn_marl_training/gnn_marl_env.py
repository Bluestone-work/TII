...
RWD_SOCIAL_CLIP          = 1.00   # r_social 单步截断（仅负值方向）
...
class IndependentRobotEnv(gym.Env):
    ...
    def get_step_result(self):
            ...
            # Near-miss penalty: only trigger when very close (< 25cm, near TB3 radius)
            # Gentle gradient to allow passage through narrow spaces (e.g. 1.2m corridors)
            near_miss_penalty = 0.0
            if effective_min < 0.25:
                near_miss_ratio = (0.25 - effective_min) / 0.25
                near_miss_penalty = -1.0 * (near_miss_ratio ** 2)  # reduced from -2.0
