    ...
    def on_episode_step(self, *, worker, base_env, policies, episode, env_index, **kwargs):
        ...
        for agent_id in episode.get_agents():
            ...
            if info:
                ...
                keys_to_track = [
                    'r_progress', 'r_static', 'r_social',
                    'r_collision', 'r_goal', 'r_time',
                ...
                for k in keys_to_track:
                    if k in info:
                        ...
                        episode.user_data["custom_metrics_history"].setdefault(metric_key, []).append(info[k])
...
GRAPH_ABLATIONS = ("social_only", "obstacle_only", "dual_graph")

ENV_CURRICULUM = {
    1: {
        "name": "Stage 1 · Circle Swap 2 车（基础避碰）",
        "map_number": 8,          # circle_swap_arena，与 run_curriculum.sh STAGE_MAP_NUM 保持同步
        "max_episode_steps": 1000,  # 8x8m 对角线 + 绕障，给足时间学避碰
        "num_obstacles": 3,
        "obs_speed_scale": 0.5,
        "description": "circle_swap 2 车对向换位 + 3 动态障碍，学习基础避碰和对称破缺。",
    },
    2: {
        "name": "Stage 2 · Circle Swap 4 车（多体交互）",
        "map_number": 8,          # circle_swap_arena，4 车 antipodal pairs
        "max_episode_steps": 1200,  # 4 车交互更复杂，多次让行/绕路，步数需更长
        "num_obstacles": 4,
        "obs_speed_scale": 0.6,
        "description": "circle_swap 4 车从对角出发，学习多体协调和动态让行。",
    },
    3: {
        "name": "Stage 3 · Circle Swap 8 车（高密度协调）",
        "map_number": 8,          # circle_swap_arena，8 车最大密度
        "max_episode_steps": 1500,  # 8 车高密度场景，频繁让行和重规划，需要最长时间
        "num_obstacles": 5,
        "obs_speed_scale": 0.7,
        "description": "circle_swap 8 车高密度场景，学习复杂多体协作和冲突解决。",
    },
    4: {
        "name": "Stage 4 · 十字交汇（场景泛化）",
        "map_number": 4,          # intersection，场景泛化测试
        "max_episode_steps": 800,  # 12x12 小地图，单程约 420 步
        "num_obstacles": 3,
        "obs_speed_scale": 0.6,
        "description": "intersection 十字路口泛化测试，验证学到的避碰策略可迁移。",
    },
}
...
def _run_training(
    ...
    if init_checkpoint:
        ...
        # Always use donor-only (weights transfer) to avoid optimizer state shape
        # mismatch when obs_dim changes between curriculum stages.
        try:
            donor = config.build()
            donor.restore(init_checkpoint)
            algo.set_weights(donor.get_weights())
            donor.stop()
            print("权重迁移成功（跳过优化器状态，防止跨阶段 shape 不匹配）")
        except Exception as e:
            print(f"权重迁移失败，从零开始训练: {e}")
...
def _resolve_ppo_training_kwargs(args):
    """根据动作空间与 profile 选择 PPO 超参数（支持离散专用模板）。"""
    action_mode = str(getattr(args, "action_mode", "continuous")).strip().lower()
    profile = str(getattr(args, "ppo_profile", "auto")).strip().lower()

    if profile not in ("auto", "continuous", "discrete"):
        raise ValueError(f"Unsupported --ppo_profile: {profile}")

    use_discrete_profile = (
        profile == "discrete"
        or (profile == "auto" and action_mode == "discrete_primitive")
    )

    if use_discrete_profile:
        # 离散动作专用：保留探索，但避免后期把已学好的避碰策略“抖散”。
        cfg = {
            "clip_param": 0.15,
            "entropy_coeff": 0.004,
            "vf_clip_param": 80.0,
        ...
        resolved_profile = "discrete_specialized"
    else:
        cfg = {
            "clip_param": 0.2,
            # Entropy schedule: starts at 0.003 for exploration, decays linearly to 0.0005
            # by 250K steps to prevent log_std from exploding (combined with free_log_std=True)
            "entropy_coeff": 0.003,
        ...
        resolved_profile = "continuous_baseline"
    ...
    return cfg, resolved_profile, use_discrete_profile
...
def _build_ppo_config(args, env_config, model_name, model_cfg):
    ...
    config = (
        ...
        .env_runners(
            ...
            rollout_fragment_length=max(50, int(args.rollout_fragment_length)),
        ...
        .training(
            ...
            model=model_cfg,
            **({"entropy_coeff_schedule": ppo_kwargs["entropy_coeff_schedule"]}
               if ppo_kwargs.get("entropy_coeff_schedule") else {}),
        )
        .multi_agent(
            policies={
                policy_name: (
...
def main():
    ...
    parser.add_argument("--action_mode",      type=str,   default="discrete_primitive",
                        choices=["discrete_primitive", "continuous"],
                        help="动作空间模式：discrete_primitive(离散动作原语) / continuous(连续v,w)")
    parser.add_argument("--ppo_profile",      type=str,   default="auto",
                        choices=["auto", "discrete", "continuous"],
                        help="PPO超参模板：auto(按action_mode自动选) / discrete / continuous")
    ...
    parser.add_argument("--safety_filter_enable", type=int, default=0,
                        help="是否启用动作级安全过滤器(0/1)")
    parser.add_argument("--allow_shield_during_training", type=int, default=0,
                        help="显式允许训练阶段启用 safety shield；默认关闭以避免破坏 PPO 的 on-policy 假设")
    parser.add_argument("--shield_front_slow_dist", type=float, default=0.50,
                        help="前方障碍距离低于该值时限速")
    parser.add_argument("--shield_front_stop_dist", type=float, default=0.20,
                        help="前方障碍距离低于该值时近似停车")
    parser.add_argument("--shield_neighbor_slow_dist", type=float, default=0.35,
                        help="邻车距离低于该值时限速")
    parser.add_argument("--shield_linear_slow", type=float, default=0.12,
                        help="shield 限速线速度")
    parser.add_argument("--shield_linear_stop", type=float, default=0.04,
                        help="shield 极限线速度")
    parser.add_argument("--shield_turn_bias", type=float, default=0.35,
                        help="前方受阻时附加转向偏置")
    parser.add_argument("--turn_in_place_front_dist", type=float, default=0.35,
                        help="前方受阻且需要大转角时触发原地转向")
    parser.add_argument("--turn_in_place_angle_thresh", type=float, default=0.45,
                        help="触发原地转向的目标角阈值")
    parser.add_argument("--turn_in_place_w", type=float, default=0.90,
                        help="原地转向角速度")
    ...
    parser.add_argument("--team_reward_lambda", type=float, default=0.7,
                        help="团队奖励混合系数")
    ...
    parser.add_argument("--gat_actor_graph", type=str, default="local_risk",
                        ...
                        help="GAT actor 的图节点来源：social_risk/local_risk=本机局部风险token，neighbor=邻居通信token")
    parser.add_argument("--graph_ablation", type=str, default="dual_graph",
                        choices=list(GRAPH_ABLATIONS),
                        help="图结构消融：social_only=仅机器人/社交图，obstacle_only=仅环境障碍图，dual_graph=社交+障碍共同建图")
    parser.add_argument("--skip_ray_stop", action="store_true",
                        help="并行实验用：启动前不执行全局 ray stop --force，避免杀掉其他训练")
    parser.add_argument("--ray_temp_dir", type=str, default="",
                        help="Ray 临时目录；并行实验请使用不同目录")
    parser.add_argument("--ray_num_cpus", type=int, default=0,
                        help="Ray num_cpus 覆盖；<=0 时使用 num_workers+1")
    parser.add_argument("--ray_num_gpus", type=float, default=-1.0,
                        help="Ray num_gpus 覆盖；<0 时不显式设置")
    parser.add_argument("--ray_object_store_memory_mb", type=int, default=0,
                        help="Ray object store memory MB；<=0 时使用默认值")
    ...
    requested_shield_enable = bool(int(args.safety_filter_enable))
    allow_shield_during_training = bool(int(args.allow_shield_during_training))
    effective_shield_enable = requested_shield_enable and allow_shield_during_training
    if requested_shield_enable and not effective_shield_enable:
        print(
            "⚠️ 已请求启用 safety shield，但训练阶段默认强制关闭。"
            "shield 会修改实际执行动作，与 PPO 记录的原始动作不一致，"
            "会破坏 on-policy 信号。若你确实要做 shield-in-the-loop 训练，"
            "请同时传 --allow_shield_during_training 1。"
        )
    resume_path = None
    ...
    predictive_feature_dim = 0  # removed from obs
    gap_feature_dim = 0  # removed from obs
    base_safety_feature_dim = 2  # front_min + min_dist only
    neighbor_prediction_top_k = 2
    neighbor_prediction_feature_dim = 7  # raw token
    ...
    agent_id_dim = 8  # one-hot agent identity for symmetry breaking
    local_obs_dim = (
        target_obs_dim
        + 2  # velocity
    ...
    if resume_path and local_obs_dim != 53:
        print(
    ...
    mode_tag = "Disc" if args.action_mode == "discrete_primitive" else "Cont"

    # ── 初始化 Ray ────────────────────────────────────────────────────────
    if not bool(getattr(args, "skip_ray_stop", False)):
        try:
            subprocess.run(
                ["ray", "stop", "--force"],
                check=False,
        ...
        print("ℹ️ 并行安全模式：跳过全局 ray stop --force")
    ...
    if int(args.ray_num_cpus) > 0:
        ray_init_kwargs["num_cpus"] = int(args.ray_num_cpus)
    if float(args.ray_num_gpus) >= 0.0:
        ray_init_kwargs["num_gpus"] = float(args.ray_num_gpus)
    if int(args.ray_object_store_memory_mb) > 0:
        ray_init_kwargs["object_store_memory"] = int(args.ray_object_store_memory_mb) * 1024 * 1024
    if str(args.ray_temp_dir).strip():
        ray_init_kwargs["_temp_dir"] = os.path.abspath(os.path.expanduser(str(args.ray_temp_dir)))
    ...
    if args.model_type == "mlp":
        ...
        print(
            f"  控制方式:    纯端到端RL"
            f"（goal-directed，shield={'ON' if effective_shield_enable else 'OFF'}，"
            f"path-tracking reward=OFF）"
        )
        ...
        env_config = {
            ...
            "enable_local_map":    False,  # MLP model 不支持 local_map CNN
            "comm_mode":           "decentralized",
            "comm_dropout_prob":   1.0,   # 无意义，但保持接口一致
            "comm_latency_steps":  1,
            "comm_jitter_steps":   0,
            "comm_noise_std":      0.0,
            # 纯端到端 RL：不再追 rolling subgoal，不再使用动作级 shield
            "reset_on_collision_event": True,
            "collision_hard_dist": 0.22,
            "collision_persist_dist": 0.24,
            "collision_persist_steps": 2,
            "shield_enable": effective_shield_enable,
            "shield_front_slow_dist": args.shield_front_slow_dist,
            "shield_front_stop_dist": args.shield_front_stop_dist,
            "shield_neighbor_slow_dist": args.shield_neighbor_slow_dist,
            "shield_linear_slow": args.shield_linear_slow,
            "shield_linear_stop": args.shield_linear_stop,
            "shield_turn_bias": args.shield_turn_bias,
            "turn_in_place_front_dist": args.turn_in_place_front_dist,
            "turn_in_place_angle_thresh": args.turn_in_place_angle_thresh,
            "turn_in_place_w": args.turn_in_place_w,
            ...
            # 到达目标必须是压倒性的主导信号：原值 24 < 刷满 2000 步 heading 奖励(~110)，
            # 导致智能体学会"对着目标慢慢蹭、刷满步数"而非真正到达。提高到 60 让"尽快到达"占优。
            "goal_reward": 60.0,
            "collision_penalty": 20.0,
            # 引入温和时间惩罚（每步 -0.008，800 步累计约 -6.4），抵消逐步刷分动机，
            # 但不至于压过 progress/goal 等正向信号；配合缩短后的 max_episode_steps 生效。
            "time_penalty": 0.008,
            ...
            # 正统 partial-done：agent 终止后退出当前 episode，不立即重生
        ...
        if args.env_stage < max(ENV_CURRICULUM):
            ...
            print(f"     --action_mode {args.action_mode} --ppo_profile {args.ppo_profile}")
        else:
    ...
    # ════════════════════════════════════════════════════════════════════
        ...
        if actor_uses_neighbor_graph:
            stage_name = "通信版 social-risk 图"
        else:
            stage_name = "局部风险注意力 (通信版 token)"
        print(f"{'='*80}")
        ...
        print(f"  图消融:       {args.graph_ablation}  (dual_graph=双分支 social+obstacle GAT 融合)")
        ...
        env_config = {
            ...
            # 纯端到端 RL：不再追 rolling subgoal；shield 默认仅用于评估/部署，不建议直接并入训练
            "reset_on_collision_event": True,
            "collision_hard_dist": 0.22,
            "collision_persist_dist": 0.24,
            "collision_persist_steps": 2,
        ...
        model_cfg = {
            ...
            "custom_model_config": {
                ...
                "base_safety_feature_dim": base_safety_feature_dim,
            ...
            "max_seq_len": 32,
            # Continuous action: state-independent log_std prevents entropy explosion
            "free_log_std": True,
        }
