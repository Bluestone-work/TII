...
class PathTrackingUtils:
    ...
    @classmethod
    def _build_prefix_lengths(cls, path_points):
        prefix = [0.0]
        for idx in range(len(path_points) - 1):
            prefix.append(prefix[-1] + cls._segment_length(path_points[idx], path_points[idx + 1]))
        return prefix
    ...
    def _select_projection_candidate(
        ...
        anchor_arc_progress=None,
        max_backward_progress=0.35,
    ):
        last_seg_idx = max(0, len(path_points) - 2)
        idx_start = int(np.clip(idx_start, 0, last_seg_idx))
        idx_end = int(np.clip(idx_end, idx_start, last_seg_idx))
        ...
        for idx in range(idx_start, idx_end + 1):
            ...
            arc_progress = prefix_lengths[idx] + t * seg_len
            lateral = float(math.hypot(point[0] - proj[0], point[1] - proj[1]))

            score = lateral
            if anchor_segment_index is not None:
                score += 0.02 * abs(int(idx) - int(anchor_segment_index))
            if anchor_arc_progress is not None:
                backward_gap = float(anchor_arc_progress) - arc_progress
                if backward_gap > max_backward_progress:
                    score += 1.0 + 4.0 * (backward_gap - max_backward_progress)
            ...
            if (
                ...
                or (
                    ...
                    and cand['arc_progress'] > best['arc_progress']
                )
            ):
                best = cand
                best_score = score
    ...
    @classmethod
    def get_path_projection(
        cls,
        point,
        path_points,
        anchor_segment_index=None,
        anchor_arc_progress=None,
        backtrack_segments=2,
        forward_search_segments=12,
        max_backward_progress=0.35,
        relocalize_lateral_error=1.25,
    ):
        if not path_points:
            return {
        ...
        prefix_lengths = cls._build_prefix_lengths(path_points)
        last_seg_idx = len(path_points) - 2
        if anchor_segment_index is None:
            return cls._select_projection_candidate(
                point,
                path_points,
                prefix_lengths,
                0,
        ...
        anchor_segment_index = int(np.clip(int(anchor_segment_index), 0, last_seg_idx))
        local_start = max(0, anchor_segment_index - int(max(0, backtrack_segments)))
        if forward_search_segments is None:
            local_end = last_seg_idx
        else:
            local_end = min(
                last_seg_idx,
                anchor_segment_index + int(max(0, forward_search_segments)),
            )

        local_best = cls._select_projection_candidate(
            point,
            path_points,
            prefix_lengths,
            local_start,
            ...
            anchor_arc_progress=anchor_arc_progress,
            max_backward_progress=max_backward_progress,
        )
        global_best = cls._select_projection_candidate(
            point,
            path_points,
            prefix_lengths,
            0,
        ...
        if global_best is None:
            return local_best
        if local_best['lateral_error'] <= relocalize_lateral_error:
            return local_best
        if global_best['lateral_error'] + 0.10 < local_best['lateral_error']:
            return global_best
        return local_best

    @classmethod
    def get_rolling_subgoal(cls, point, path_points, lookahead_dist=0.8, **projection_kwargs):
        if not path_points:
            return {
                'subgoal': tuple(point),
                'projection': tuple(point),
        ...
        proj_info = cls.get_path_projection(point, path_points, **projection_kwargs)
