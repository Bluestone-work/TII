...
class RobotPolicyNode(Node):
    ...
    def __init__(self):
        ...
        self.current_projection: Optional[Tuple[float, float]] = None
        ...
        self._committed_subgoal_target: Optional[Tuple[float, float]] = None
    ...
    def _build_local_obs(self, target_override: Optional[Tuple[float, float]] = None) -> Optional[np.ndarray]:
        ...
        front_blocked = 1.0 if front_min < self.corner_escape_front_dist else 0.0
    ...
    def _commit_subgoal(
        ...
        self._committed_subgoal_target = (float(target[0]), float(target[1]))
    ...
    def _get_committed_subgoal(self, mode: str, partner: str = '') -> Optional[Tuple[float, float]]:
        ...
        dist = math.hypot(
            float(self._committed_subgoal_target[0]) - float(state['x']),
            float(self._committed_subgoal_target[1]) - float(state['y']),
        )
    ...
    def _get_nominal_subgoal(self, lookahead_dist: Optional[float] = None) -> Optional[Tuple[float, float]]:
        ...
        tracking_kwargs = {}
        if len(self.waypoints) >= 2 and self.current_projection is not None:
            tracking_kwargs = {
    ...
    def _select_local_detour_subgoal(
        ...
        if conflict is not None:
            partner = str(int(conflict['partner']))
            conflict_dist = float(conflict.get('dist', float('inf')))
            backoff_needed = (
                conflict_dist < self.yielding_hard_stop_dist
                or (front_min < self.subgoal_block_front_dist and side_clear < side_clear_thresh)
            )
            same_partner = partner == str(self._yield_partner)
            should_hold = (
                self._yield_hold_steps > 0
                and same_partner
            ...
            if should_hold:
                hold_mode = 'backoff' if self._committed_subgoal_mode == 'backoff' else 'yield'
                committed = self._get_committed_subgoal(hold_mode, partner=partner)
                if committed is not None:
                    self._yield_hold_steps = max(0, self._yield_hold_steps - 1)
                    return committed, hold_mode
                self._yield_hold_steps = max(0, self._yield_hold_steps - 1)
                lookahead = max(0.25, float(adaptive_lookahead))
                if backoff_needed:
                    x_body = -max(0.14, 0.55 * lookahead)
                    y_body = self._yield_turn_sign * max(0.14, 0.55 * lookahead)
                    target = self._body_to_world_point(x_body, y_body, state=my_state)
                    committed = self._commit_subgoal(target, 'backoff', self.yielding_commit_steps, partner=partner)
                    return committed, 'backoff'
                x_body = max(0.08, 0.35 * lookahead)
                y_body = self._yield_turn_sign * max(0.18, 0.80 * lookahead)
                target = self._body_to_world_point(x_body, y_body, state=my_state)
                committed = self._commit_subgoal(target, 'yield', self.yielding_commit_steps, partner=partner)
                return committed, 'yield'

            if (
                float(conflict['should_yield']) > 0.5
                and conflict_dist < self.yielding_soft_dist
                and (not math.isfinite(float(conflict['ttc'])) or float(conflict['ttc']) < self.yielding_ttc)
            ):
                self._yield_partner = partner
    ...
    def _get_tracking_target(self) -> Optional[Tuple[float, float]]:
        ...
        try:
            if self.lookahead_dist <= 0.0:
                proj = PathTrackingUtils.get_path_projection(pos, self.waypoints, **tracking_kwargs)
                ...
                if len(self.waypoints) >= 2:
                    i = int(np.clip(seg_idx, 0, len(self.waypoints) - 2))
                    a, b = self.waypoints[i], self.waypoints[i + 1]
                    self.current_path_heading = float(math.atan2(b[1] - a[1], b[0] - a[0]))
                    self.current_waypoint_index = int(np.clip(i + 1, 0, len(self.waypoints) - 1))
                else:
                    self.current_path_heading = 0.0
                    self.current_waypoint_index = 0
            ...
            self.current_waypoint_index = int(np.clip(seg_idx + 1, 0, max(len(self.waypoints) - 1, 0)))
    ...
    def _control_loop(self):
        ...
        # 与训练环境保持一致：正向动作映射到前进速度，负向动作映射到后退速度。
        a_lin = float(np.clip(action[0], -1.0, 1.0))
        if a_lin >= 0.0:
            linear_vel = a_lin * self.max_forward_vel
        else:
            linear_vel = a_lin * self.max_reverse_vel
        angular_vel = float(np.clip(action[1], -1.0, 1.0) * self.max_angular_vel)
        ...
        # 5. 发布 cmd_vel
        cmd = Twist()
        cmd.linear.x  = float(np.clip(linear_vel, -self.max_reverse_vel, self.max_forward_vel))
        cmd.angular.z = float(np.clip(angular_vel, -self.max_angular_vel, self.max_angular_vel))
